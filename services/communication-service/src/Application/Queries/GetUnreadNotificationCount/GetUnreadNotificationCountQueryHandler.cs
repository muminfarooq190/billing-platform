...
public sealed class GetUnreadNotificationCountQueryHandler(IReadDbConnectionFactory connectionFactory) : IRequestHandler<GetUnreadNotificationCountQuery, int>
{
    public async Task<int> Handle(GetUnreadNotificationCountQuery request, CancellationToken cancellationToken)
    {
        ...
        return await dbConnection.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM notifications WHERE recipient_id = @RecipientId AND read_at IS NULL",
