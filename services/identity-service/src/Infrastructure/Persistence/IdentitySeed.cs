...
public static class IdentitySeed
{
    public static async Task SeedDefaultsAsync(IdentityDbContext dbContext, CancellationToken cancellationToken)
    {
        ...
        if (!await dbContext.RoleDefinitions.AnyAsync(x => x.TenantId == null, cancellationToken))
        {
            ...
            owner.SetPermissions(new[] { "identity.users.manage", "identity.roles.manage", "identity.audit.read", "identity.settings.manage", "branding.theme.manage", "travel.quotation.read", "travel.quotation.write", "billing.invoices.read" });
            var admin = RoleDefinition.Create(null, "Admin", "System admin role.", true);
            admin.SetPermissions(new[] { "identity.users.manage", "identity.audit.read", "identity.settings.manage", "branding.theme.manage", "travel.quotation.read", "travel.quotation.write", "billing.invoices.read" });
