...
public sealed class IdentityAuditController(IdentityDbContext dbContext, IFeatureGate featureGate, ITenantContext tenantContext) : ControllerBase
{
    ...
    public async Task<IActionResult> GetAudit(Guid userId, CancellationToken cancellationToken)
    {
        ...
        await featureGate.EnsureEnabledAsync(FeatureKeys.IdentityAuditRead, tenantId, tenantContext.UserId, cancellationToken);
    ...
    {
        var tenantId = ResolveTenantId();
        await featureGate.EnsureEnabledAsync(FeatureKeys.IdentityAuditExport, tenantId, tenantContext.UserId, cancellationToken);
