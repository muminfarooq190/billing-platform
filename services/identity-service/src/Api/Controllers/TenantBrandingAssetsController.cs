...
public sealed class TenantBrandingAssetsController(IdentityDbContext dbContext, IBrandAssetStorage storage, IFeatureGate featureGate, ITenantContext tenantContext) : ControllerBase
{
    [HttpGet("{**storageKey}")]
    public async Task<IActionResult> Read(string storageKey, CancellationToken cancellationToken)
    {
        var tenantId = ResolveTenantId();
        await featureGate.EnsureEnabledAsync(FeatureKeys.BrandingAssetsManage, tenantId, tenantContext.UserId, cancellationToken);
