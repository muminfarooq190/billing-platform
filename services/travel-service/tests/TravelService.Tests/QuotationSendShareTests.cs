...
public sealed class QuotationSendShareTests
{
    ...
    public async Task SendQuotation_ShouldCreateShareLink_AndMarkQuotationSent()
    {
        ...
        attachmentRepository.Items.Should().ContainSingle(x => x.QuotationRevisionId == revision.Id && x.AttachmentType == "Pdf");
    ...
    private sealed class InMemoryQuotationShareLinkRepository(params QuotationShareLink[] shareLinks) : IQuotationShareLinkRepository
    {
        ...
        public Task AddAsync(QuotationShareLink shareLink, CancellationToken cancellationToken) { ShareLinks.Add(shareLink); return Task.CompletedTask; }
    ...
    {
        public List<QuotationAttachment> Items { get; } = [];
        public Task AddAsync(QuotationAttachment attachment, CancellationToken cancellationToken) { Items.Add(attachment); return Task.CompletedTask; }
        public Task<QuotationAttachment?> GetByIdAsync(Guid quotationId, Guid attachmentId, CancellationToken cancellationToken) => Task.FromResult(Items.SingleOrDefault(x => x.QuotationId == quotationId && x.Id == attachmentId));
        public Task<IReadOnlyList<QuotationAttachment>> ListByQuotationIdAsync(Guid quotationId, CancellationToken cancellationToken) => Task.FromResult<IReadOnlyList<QuotationAttachment>>(Items.Where(x => x.QuotationId == quotationId).ToList());
    ...
    private sealed class InMemoryQuotationStatusHistoryRepository : IQuotationStatusHistoryRepository
    {
        ...
        public Task AddAsync(QuotationStatusHistory history, CancellationToken cancellationToken) { Items.Add(history); return Task.CompletedTask; }
