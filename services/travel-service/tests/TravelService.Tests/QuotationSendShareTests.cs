...

{
...

    {
        ...
        var handler = new SendQuotationCommandHandler(quotationRepository, revisionRepository, shareLinkRepository, historyRepository, new InMemoryQuotationApprovalRequestRepository(), new NoOpCommunicationWorkflowClient(), new AllowAllFeatureGate(), new NoOpActivityWriter(), new FakeActorContext(quotation.TenantId), new NoOpUnitOfWork(), new FakeTenantContext());
