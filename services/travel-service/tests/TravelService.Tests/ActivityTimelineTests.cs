...
public sealed class ActivityTimelineTests
{
    ...
    public async Task CreateQuotationRevision_ShouldWriteTimelineEntry()
    {
        ...
        var handler = new CreateQuotationRevisionCommandHandler(new InMemoryQuotationRepository(quotation), new InMemoryQuotationRevisionRepository(), new AllowAllFeatureGate(), activityWriter, new NoOpAuditWriter(), new FakeActorContext(quotation.TenantId), new NoOpUnitOfWork(), new FakeTenantContext());
    ...
    public async Task CreateBookingFromQuotation_ShouldWriteBookingAndQuotationTimelineEntries()
    {
        ...
        var activityWriter = new RecordingActivityWriter();
        var handler = new CreateBookingFromQuotationCommandHandler(new InMemoryQuotationRepository(quotation), new InMemoryQuotationRevisionRepository(revision), new InMemoryBookingRepository(), new InMemoryBookingStatusHistoryRepository(), new AllowAllFeatureGate(), activityWriter, new NoOpUnitOfWork(), new FakeTenantContext());
