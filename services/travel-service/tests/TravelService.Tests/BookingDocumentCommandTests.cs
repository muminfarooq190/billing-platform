...
public sealed class BookingDocumentCommandTests
{
    ...
    public async Task UploadBookingDocument_ShouldPersistDocument_AndUploadFile()
    {
        ...
        var handler = new UploadBookingDocumentCommandHandler(new InMemoryBookingRepository(booking), new InMemoryTravelerRepository(traveler), documentRepository, fileStorage, new AllowAllFeatureGate(), new NoOpActivityWriter(), new FakeActorContext(booking.TenantId), new NoOpUnitOfWork(), new FakeTenantContext());
