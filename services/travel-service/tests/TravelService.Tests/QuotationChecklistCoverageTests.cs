...
public sealed class QuotationChecklistCoverageTests
{
    ...
    public async Task CreateRevision_ListRevisions_SendQuote_ShouldSupportHappyPathFlow()
    {
        ...
        var createRevisionHandler = new CreateQuotationRevisionCommandHandler(quotationRepository, revisionRepository, new AllowAllFeatureGate(), new NoOpActivityWriter(), new NoOpAuditWriter(), new FakeActorContext(quotation.TenantId), new NoOpUnitOfWork(), new FakeTenantContext());
        ...
        var sendHandler = new SendQuotationCommandHandler(quotationRepository, revisionRepository, shareLinkRepository, historyRepository, new InMemoryQuotationApprovalRequestRepository(), new AllowAllFeatureGate(), new NoOpActivityWriter(), new FakeActorContext(quotation.TenantId), new NoOpUnitOfWork(), new FakeTenantContext());
