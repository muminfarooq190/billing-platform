...
public sealed class QuotationAttachmentCommandTests
{
    ...
    public async Task UploadQuotationAttachment_ShouldPersistAttachment_AndUploadFile()
    {
        ...
        var handler = new UploadQuotationAttachmentCommandHandler(quotationRepository, revisionRepository, attachmentRepository, fileStorage, new AllowAllFeatureGate(), new NoOpActivityWriter(), new FakeActorContext(quotation.TenantId), new NoOpUnitOfWork(), new FakeTenantContext());
